<?php /* Template Name: Расписание преподавателей */?>
<? get_header(); ?>
<?php
       ...
       if(is_usr_admin()){
         ...
         if(GETA('action','save_edit')){
           ...
           $wpdb->update( 'timetable_teachers',
            array( 'day' => $_POST['weekday'], 'time' => $_POST['time'] ),
            array( 'id' => $_POST['id'] )
          );
         }
         if(GETA('action','save_add')){
           $wpdb->insert(
              'timetable_teachers',
              array( 'id_teacher' => $_POST['teachers'],'day'=>$_POST['weekday'],'time'=>$_POST['time'])
            );
         ...
         if(GETA('action','edit')){
           ...
           ?>
            <form action="?action=save_edit" method="post" accept-charset="utf-8">
              ...
              <?  
                $day = $wpdb->get_results("SELECT * FROM timetable_teachers WHERE id=".$_GET['id']);
                $day = $day[0];
                $teacher = getTeacherById($day->id_teacher);
                echo $teacher->firstname.' '.$teacher->lastname.' '.$teacher->patronymic;
              ?>
              (День:<? echo getDayByNumber($day->day);?>,Время:<? echo $day->time;?>)
        ...
        <?
          ...
          foreach ($array_time as $key) {
            ?>
              <tr>
                <td><? echo $key;?></td>
                <td><? timetable_teacher(1,$key)?></td>
                <td><? timetable_teacher(2,$key)?></td>
                <td><? timetable_teacher(3,$key)?></td>
                <td><? timetable_teacher(4,$key)?></td>
                <td><? timetable_teacher(5,$key)?></td>
              </tr>
            <? 
          }
        ?>
